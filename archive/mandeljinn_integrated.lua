...
function apply_encoder_movement()
  ...
  if changed then
    render_needed = true
    -- Update orbit for new position for audio
...
function init()
  ...
  -- Resend orbit data after audio starts to ensure synth receives updates
  calculate_and_send_orbit(center_x, center_y)
  
  -- Timer disabled to avoid threading issues
  -- main_timer = metro.init()
...
function enc(n, delta)
  ...
  -- Trigger updates since timer is disabled
  apply_encoder_movement()
  start_render_if_needed()
  step_render()
  redraw()
